...
contract MultiSigWallet {
    ...
    function swapBNBToBUSD() public onlyOwner {
        ...
        uint256[] memory amount = routerhelper.getAmountsOut(
            address(this).balance,
            path
        );
        routerhelper.swapExactETHForTokens{value: amount[0]}(
            amount[1],
            path,
            address(this),
            block.timestamp
        ...
        )
    {
        ...
        return (trans.to, trans.value, trans.numConfirmations);
    }
    ...
    function deleteTranscation() external onlyOwner {
        require(transaction.to == address(0), "tx dont exist");
        delete transaction;
    }
}
